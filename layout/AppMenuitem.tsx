...
const AppMenuitem = (props: AppMenuItemProps) => {
    ...
    useSubmenuOverlayPosition({
        ...
        when:
            props.root &&
            active &&
            ((isSlim && isSlim()) || isSlimPlus() || isHorizontal()) &&
            isDesktop(),
    });
    ...
    useEffect(() => {
        if (!((isSlim && isSlim()) || isSlimPlus() || isHorizontal()) && isActiveRoute) {
            setActiveMenu(key);
        }
        const url = (pathname || '') + (searchParams?.toString() || '');
        const onRouteChange = () => {
            if (!((isSlim && isSlim()) || isHorizontal()) && item!.to && item!.to === url) {
                setActiveMenu(key);
            }
        };
        onRouteChange();
    ...
    const itemClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        // navigate with hover
        if (props.root && ((isSlim && isSlim()) || isHorizontal() || isSlimPlus())) {
            const isSubmenu =
        ...
        if (item?.items) {
            ...
            if (
                props.root &&
                !active &&
                ((isSlim && isSlim()) || isHorizontal() || isSlimPlus())
            ) {
                setLayoutState((prevLayoutState) => ({
                    ...prevLayoutState,
